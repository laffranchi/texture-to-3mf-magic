...
function geometryToMeshXML(geometry: THREE.BufferGeometry, objectId: number): string {
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  ...
  // Build objects XML
  const objectsXml = meshes.map((mesh, idx) => 
    geometryToMeshXML(mesh.geometry, idx + 1)
  ).join('\n');

  // Build items (references to objects) with transforms
  const itemsXml = meshes.map((mesh, idx) => 
    `    <item objectid="${idx + 1}" />`
  ).join('\n');
